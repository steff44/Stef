(function () {
  ...
  /* ---------- Page Agenda : calendrier mois / semaine / jour ---------- */
  const agendaPage = document.querySelector("[data-agenda-page]");
  if (agendaPage) {
    const MOIS_FR = [
    ...
    function inRange(iso, debut, fin) {
      return iso >= debut && iso <= fin;
    }
    function pillClass(item) {
      if (item.type === "vacances") return "agenda-pill--vacances";
      if (item.type === "ferie") return "agenda-pill--ferie";
      return "agenda-pill--" + item.categorie;
    }

    function itemsForDay(date, categorieFiltre) {
      const iso = toISO(date);
      const items = [];
      CLUB_DATA.agenda.vacances.forEach(function (v) {
        if (inRange(iso, v.debut, v.fin)) items.push({ type: "vacances", titre: v.titre });
      });
      CLUB_DATA.agenda.feries.forEach(function (f) {
        if (f.date === iso) items.push({ type: "ferie", titre: f.titre });
      });
      CLUB_DATA.agenda.evenements.forEach(function (e) {
        const correspond = e.date ? e.date === iso : inRange(iso, e.dateDebut, e.dateFin);
        if (correspond && (!categorieFiltre || e.categorie === categorieFiltre)) {
          items.push({ type: "evenement", titre: e.titre, categorie: e.categorie, heure: e.heure, lieu: e.lieu });
        }
      });
      CLUB_DATA.agenda.recurrents.forEach(function (r) {
        if (date.getDay() === r.jourSemaine && (!categorieFiltre || r.categorie === categorieFiltre)) {
          items.push({ type: "evenement", titre: r.titre, categorie: r.categorie, heure: r.heure, lieu: r.lieu });
        }
      });
      return items;
    }
    ...
    function buildMonthCells(year, month) {
      ...
      const totalDays = startOffset + new Date(year, month + 1, 0).getDate();
    ...
    const periodLabel = document.querySelector("[data-agenda-period]");
    const categorieSelect = document.querySelector("[data-agenda-categorie]");
    const viewButtons = document.querySelectorAll("[data-agenda-view]");
    const today = new Date();
    let currentView = "mois";
    let currentDate = new Date();

    CLUB_DATA.agenda.categories.forEach(function (cat) {
    ...
    function setActiveViewButton() {
      viewButtons.forEach(function (b) {
        b.classList.toggle("is-active", b.dataset.agendaView === currentView);
      });
    }

    function renderGrid(cells, extraClass) {
      const headHTML = '<div class="agenda-grid-head">' +
        JOURS_FR.map(function (j) { return "<span>" + j + "</span>"; }).join("") +
        "</div>";
      const monthRef = currentView === "mois" ? currentDate.getMonth() : null;
      const cellsHTML = cells.map(function (d) {
        const outside = monthRef !== null && d.getMonth() !== monthRef ? " is-outside" : "";
        const isToday = sameDay(d, today) ? " is-today" : "";
        const items = itemsForDay(d, categorieSelect.value);
        return '<div class="agenda-day' + outside + isToday + '" data-agenda-day="' + toISO(d) + '">' +
          '<span class="agenda-day-number">' + d.getDate() + "</span>" +
          items.map(function (item) {
            return '<span class="agenda-pill ' + pillClass(item) + '">' + item.titre + "</span>";
          }).join("") +
          "</div>";
      }).join("");
      body.innerHTML = headHTML + '<div class="agenda-grid' + (extraClass ? " " + extraClass : "") + '">' + cellsHTML + "</div>";
      body.querySelectorAll("[data-agenda-day]").forEach(function (el) {
        el.addEventListener("click", function () {
          const parts = el.getAttribute("data-agenda-day").split("-").map(Number);
    ...
    function renderDayList() {
      const items = itemsForDay(currentDate, categorieSelect.value);
      if (!items.length) {
        body.innerHTML = '<p class="agenda-day-list-empty">Aucun événement ce jour-là.</p>';
        return;
      }
      body.innerHTML = '<div class="agenda-day-list">' +
        items.map(function (item) {
          const details = [item.heure, item.lieu].filter(Boolean).join(" · ");
          const badge = item.categorie || (item.type === "vacances" ? "Vacances" : "Jour férié");
          return '<div class="agenda-day-list-item">' +
            '<span class="agenda-pill ' + pillClass(item) + '">' + badge + "</span>" +
            "<div><strong>" + item.titre + "</strong>" + (details ? "<span>" + details + "</span>" : "") + "</div>" +
            "</div>";
        }).join("") +
        "</div>";
    }
    ...
    function render() {
      if (currentView === "mois") {
        ...
        periodLabel.textContent = MOIS_FR[month] + " " + year;
        renderGrid(buildMonthCells(year, month), "");
      } else if (currentView === "semaine") {
        const start = startOfWeek(currentDate);
        const cells = [];
        for (let i = 0; i < 7; i++) cells.push(new Date(start.getFullYear(), start.getMonth(), start.getDate() + i));
        const end = cells[6];
        const memeMois = start.getMonth() === end.getMonth();
        periodLabel.textContent = start.getDate() + (memeMois ? "" : " " + MOIS_FR[start.getMonth()]) +
          " – " + end.getDate() + " " + MOIS_FR[end.getMonth()] + " " + end.getFullYear();
        renderGrid(cells, "agenda-week");
      } else {
        periodLabel.textContent = JOURS_FR_LONG[currentDate.getDay()] + " " + currentDate.getDate() + " " +
          MOIS_FR[currentDate.getMonth()].toLowerCase() + " " + currentDate.getFullYear();
        renderDayList();
      }
    }
    ...
    viewButtons.forEach(function (btn) {
      btn.addEventListener("click", function () {
        currentView = btn.dataset.agendaView;
        setActiveViewButton();
        render();
      });
    });
    categorieSelect.addEventListener("change", render);
    document.querySelector("[data-agenda-prev]").addEventListener("click", function () { shiftPeriod(-1); });
    document.querySelector("[data-agenda-next]").addEventListener("click", function () { shiftPeriod(1); });
